...
var retargeting_priority = 2
func _init():
	self.changes_targeting = true
	self.icon = stress_icon
	self.attack_modifier = 0.9
	self.magic_attack_modifier = 0.9
...
func get_enemies(actor, actors: Array):
	return actors.filter(func(a): return a.is_alive and a != actor)
