...
func _on_chalice_player_interaction(proxy: PlayerProxy):
	...
	if VarsService.get_flag("kadath.right_switch.chalice"):
...
func attempt_chalice_interaction(proxy: PlayerProxy):
	# Disable menus, disable input
	var chalice = $Chalice
	
	await self.move_player_to_chalice(proxy, chalice)
	
	var responses = await DialogService.open_dialog("chalice_right")
	...
	if responses[0] == RESPONSE_YES:
		...
		VarsService.set_flag("kadath.right_switch.chalice", true)
		animation_player.play("chalice_activate")
		await animation_player.animation_finished
		await DialogService.open_dialog("after_chalice_battle")

func move_player_to_chalice(proxy, chalice):
