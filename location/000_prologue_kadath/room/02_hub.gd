...
func spawn_pickman():
	...
	$NPCPickman.global_position.x = EntitiesService.proxy.global_position.x

func unlock_pickman():
	$JoinTrigger.monitoring = false
	EntitiesService.party.set_unlocked(PartyMember.Id.PICKMAN, true)
	EntitiesService.party.reset_movement()
	
func _on_join_trigger_body_entered(_body):
	CutsceneService.play_cutscene_from_file("res://location/000_prologue_kadath/cutscene/catch_up.txt")
