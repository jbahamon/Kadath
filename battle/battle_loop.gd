...
func initialize(init_actors: Array, init_ui: BattleUI):
	...
	for event in BattleService.Event.values():
		observers[event] = []
	self.end_result = null
	
func do_battle():

	var battle_end_state = BattleEndState.new()
	...
	while true:
		...
		await self.check_deaths_and_reactions(battle_end_state)
		...
		if is_battle_won():
			...
			battle_end_state.party_actors = party_actors
			self.end_result = BattleEndState.Result.WIN
			break
			
		elif is_battle_lost():
			self.end_result = BattleEndState.Result.LOSE
			break
			
		elif self.battle_end_state.result == BattleEndState.Result.ESCAPE:
			battle_end_state.party_actors = []
			battle_end_state.enemy_actors = []
			
			for actor in actors:
				if not actor is PartyMember:
					battle_end_state.enemy_actors.append(actor)
				else:
					battle_end_state.party_actors.append(actor)
			break
		else:
	...
	battle_end_state.result = self.end_result
...
func on_actor_death(actor, battle_end_state):
	...
	if actor is PartyMember:
		...
		self.add_rewards(battle_end_state, actor.battler.rewards)
...
func add_rewards(battle_end_state, rewards: BattleRewards):
	battle_end_state.add_rewards(rewards)
...
func check_deaths_and_reactions(battle_end_state):
	...
	while should_continue:
		...
		while true: 
			...
			await self.on_actor_death(actor, battle_end_state)
